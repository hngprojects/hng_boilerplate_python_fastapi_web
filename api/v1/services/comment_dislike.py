...
class CommentDislikeService(Service):
    ...
    def create(self, db: Session, user_id, comment_id, client_ip: Optional[str] = None):
        ...
        if dislike_data:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="You can only dislike once")
        # check if comment exists
        comment = check_model_existence(db, Comment, comment_id)

        # create and add the new commentDislike to the database
        new_dislike = CommentDislike(comment_id=comment_id, user_id=user_id, ip_address=client_ip)
        db.add(new_dislike)
        db.commit()
