...
class CommentLikeService(Service):
    ...
    def create(self, db: Session, user_id, comment_id, client_ip: Optional[str] = None):
        ...
        if like_data:
            raise HTTPException(status_code=status.HTTP_200_OK, detail="You've already liked this comment")
        check_model_existence(db, Comment, comment_id)
        new_like = CommentLike(comment_id=comment_id, user_id=user_id, ip_address=client_ip)
        db.add(new_like)
        db.commit()
        db.refresh(new_like)
        return new_like
        ...
        if query_params:
            for column, value in query_params.items():
                if hasattr(CommentLike, column) and value:
                    query = query.filter(getattr(CommentLike, column).ilike(f'%{value}%'))
