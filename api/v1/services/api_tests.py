...
class PythonAPIs(unittest.TestCase):
    ...
    valid_body = {"email": "woss7@mailinator.com", "password": "Pa$$w0rd!", "first_name": fake.first_name(), "last_name": fake.last_name()}
    ...
    VALID_CREDENTIALS = {"email": "woss1@mailinator.com", "password": "Pa$$w0rd!", "first_name": fake.first_name(), "last_name": fake.last_name()}
    ...
    @classmethod
    def setUpClass(cls):
        auth_response = requests.post(f"{cls.baseUrl}/api/v1/auth/login", json={"email": "woss2@mailinator.com", "password": "Pa$$w0rd!"})
        auth_response_data = auth_response.json()
        cls.access_token = auth_response_data["data"]["access_token"]
        cls.user_id = auth_response_data["data"]["user"]["id"]
        print(auth_response_data)
        instance = cls()
        instance.assertEqual(auth_response.status_code, 200, "Expected status code 200, got {}".format(auth_response.status_code))
    ...
    def test_register_admin_successfully(self):
        ...
        self.assertIsInstance(response_data["data"]["access_token"], str)
        self.assertGreater(len(response_data["data"]["access_token"]), 0)
        print("Super Admin successfully created, status code is {}".format(response.status_code))
    ...
    def test_newsletter(self):
        response = requests.post(f"{self.baseUrl}/api/v1/newsletters", json = {"email": "woss@mailinator.com"})
        self.assertEqual(response.status_code, 200, "Expected status code 200, got {}".format(response.status_code))

    def test_change_password(self):
        response = requests.patch(f"{self.baseUrl}/api/v1/users/me/password", json = self.change_password)
        self.assertEqual(response.status_code, 200, "Expected status code 200, got {}".format(response.status_code))
    ...
    def test_get_user_by_id(self):
        ...
        response = requests.get(f"{self.baseUrl}/api/v1/users/{self.user_id}", headers=headers)
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_user_by_id(self):
        ...
        response = requests.delete(f"{self.baseUrl}/api/v1/users/{self.user_id}", headers=headers)
