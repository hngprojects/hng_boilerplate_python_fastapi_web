...
def create_new_job(job_data: JobCreate, db: Annotated[Session, Depends(get_db)], user_id: Annotated[User, Depends(get_current_user)]):
...
def get_job(job_id: str, db: Annotated[Session, Depends(get_db)], user_id: Annotated[User, Depends(get_current_user)]):
