...
@role_perm.post(
    "/organizations/{org_id}/users/{user_id}/roles", tags=["assign role to a user"]
)
def assign_role(
...
@role_perm.delete(
    "/roles/{role_id}", tags=["delete role"], response_model=ResponseModel
)
def delete_role(
    role_id: str,
    db: Session = Depends(get_db),
    ...
    if result:
        ...
        return ResponseModel(
            success=False, status_code=404, message="Role not found.", data=None
        )
