...
def assign_permission_endpoint(
    ...
    return permission_service.assign_permission_to_role(
        db, role_id, request.permission_id
    )


@perm_role.delete(
    "/permissions/{permission_id}",
    tags=["Delete permissions"],
...
@perm_role.put("/{role_id}/roles/{permission_id}", tags=["update permissions"])
def update_permission_endpoint(
    new_permission_id: PermissionUpdate,  # New Permission ID from path
    permission_id: str = Path(
        ..., description="The ID of the old permission"
    ),  # Old Permission ID from path
    role_id: str = Path(..., description="The ID of the role"),  # Role ID from path
    db: Session = Depends(get_db),
    current_user: User = Depends(user_service.get_current_super_admin),
):  # Assuming only super admins can update permissions
    return permission_service.update_permission_on_role(
        db, role_id, permission_id, new_permission_id.new_permission_id
    )
