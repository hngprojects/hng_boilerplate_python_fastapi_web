...
def existing_blog_post(mock_db_session, current_user):
    blog = Blog(id=f'{uuid7()}', title="Original Title",
                content="Original Content", author_id=current_user.id)
    mock_db_session.query(Blog).filter(
        Blog.id == blog.id).first.return_value = blog
    return blog
...
async def test_update_blog_success(client, mock_db_session, current_user, valid_blog_post, existing_blog_post):
    ...
    response = client.put(
        f"/api/v1/blogs/{existing_blog_post.id}", json=valid_blog_post.model_dump())
...
async def test_update_blog_not_found(client, mock_db_session, current_user, valid_blog_post):
    mock_db_session.query(Blog).filter(
        Blog.id == f'{uuid7()}').first.return_value = None

    app.dependency_overrides[get_db] = lambda: mock_db_session
    app.dependency_overrides[get_current_user] = lambda: current_user

    response = client.put(
        f"api/v1/blogs/{uuid7()}", json=valid_blog_post.model_dump())
...
async def test_update_blog_forbidden(client, mock_db_session, current_user, valid_blog_post, existing_blog_post):
    ...
    different_user = User(id=f'{uuid7()}', username="otheruser", email="other@example.com",
                          password="hashedpassword1", first_name="other", last_name="user")
    ...
    assert response.json() == {
        'message': 'Not authorized to update this blog', 'status_code': 403, 'success': False}
...
async def test_update_blog_empty_fields(client, mock_db_session, current_user, existing_blog_post):
    ...
    response = client.put(
        f"api/v1/blogs/{existing_blog_post.id}", json={"title": "", "content": ""})

    assert response.status_code == 400
    assert response.json() == {
        'message': 'Title and content cannot be empty', 'status_code': 400, 'success': False}
...
async def test_update_blog_internal_error(client, mock_db_session, current_user, valid_blog_post, existing_blog_post):
    ...
    assert response.json() == {
        'message': 'An error occurred while updating the blog post', 'status_code': 500, 'success': False}
