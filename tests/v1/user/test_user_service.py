...
def test_fetch_all_search(db_session, user_service):
    # Mock users
    user1 = User(id="1", first_name="John", last_name="Doe", email="john.doe@example.com", is_active=True)
    ...
    assert response.data["pagination"]["total_users"] == 1
    assert response.data["pagination"]["total_pages"] == 1

def test_fetch_all_is_active_filter(db_session, user_service):
    user1 = User(id="1", first_name="Jane", last_name="Doe", email="jane.doe@example.com", is_active=True)
    db_session.query().filter().order_by().limit().offset().all.return_value = [user1]
    db_session.query().filter().count.return_value = 1
...
def test_fetch_all_pagination(db_session, user_service):
    users = [User(id=str(i), first_name=f"User{i}", last_name="Test", email=f"user{i}@example.com", is_active=True) for i in range(25)]
