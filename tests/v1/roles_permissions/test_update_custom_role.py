...
def create_mock_role(mock_db_session, role_id, role_name, is_builtin=False):
...
def test_update_custom_role(mock_db_session, access_token):
    role_id = str(uuid7())
    create_mock_role(mock_db_session, role_id, "old_custom_role", is_builtin=False)

    headers = {"Authorization": f"Bearer {access_token}"}
    role_update = {"name": "new_custom_role", "is_builtin": False}

    response = client.put(
        f"/api/v1/custom/roles/{role_id}",
        json=role_update,
        headers=headers
    )

    assert response.status_code == 200
    assert response.json()["message"] == "Role new_custom_role updated successfully"


def test_update_custom_role_not_found(mock_db_session, access_token):
    non_existent_role_id = str(uuid7())

    # Ensure no role is returned for the non-existent role_id
    ...
    response = client.put(
        f"/api/v1/custom/roles/{non_existent_role_id}",
        json=role_update,
        headers=headers
    )
