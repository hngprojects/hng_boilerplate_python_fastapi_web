...
def error_user_deactivation(test_db):
    ...
    login = client.post(
        "/auth/login", data={"username": "testuser", "password": "Testpassword@123"}
    )
    access_token = login.json()["access_token"]

    missing_field = client.patch(
        "/api/v1/users/accounts/deactivate",
        json={"reason": "No longer need the account"},
        headers={"Authorization": f"Bearer {access_token}"},
    )
    assert missing_field.status_code == 422

    confirmation_false = client.patch(
        "/api/v1/users/accounts/deactivate",
    ...
    assert (
        confirmation_false.json()["detail"]
...
def success_deactivation_test(test_db):
    ...
    success_deactivation = client.patch(
        "/api/v1/users/accounts/deactivate",
        json={"reason": "No longer need the account", "confirmation": True},
        headers={"Authorization": f"Bearer {access_token}"},
    )
...
def test_iser_inactive(test_db):
    ...
    user_already_deactivated = client.patch(
        "/api/v1/users/accounts/deactivate",
        json={"reason": "No longer need the account", "confirmation": True},
        headers={"Authorization": f"Bearer {access_token}"},
    )
