...
def test_duplicate_email(client_with_mocks):
    ...
    data = response.json()
    inner_message = data.get('message', {}).get('message', '')
    expected_message = 'Email already registered'
    assert inner_message == expected_message, f"Expected message '{expected_message}', got '{inner_message}'"
...
def test_invalid_email(client_with_mocks):
    ...
    response = client.post(
        "/api/v1/waitlist/join", json={"email": "invalid_email", "full_name": "Test User"}
    )
    data = response.json()
...
def test_signup_with_empty_name(client_with_mocks):
    client, _ = client_with_mocks
    response = client.post(
        "/api/v1/waitlist/join", json={"email": "test@example.com", "full_name": ""}
    )
    data = response.json()
    assert response.status_code == 422
