...
def test_update_testimonial_success(mock_user_service, mock_db_session):
    """Test if the testimonial is updated successfully."""
    ...
    # Update testimonial
    updated_content = "Updated testimonial content"
    ...
    assert response.json().get("data").get("content") == updated_content

@pytest.mark.usefixtures("mock_db_session", "mock_user_service")
def test_update_testimonial_validation_error(mock_user_service, mock_db_session):
    """Test updating a testimonial with validation error."""
    ...
    # Attempt to update testimonial with invalid content
    response = client.patch(
        f'/api/v1/testimonials/{testimonial.id}',
        json={"content": ""},
        headers={'Authorization': f'Bearer {access_token}'}
